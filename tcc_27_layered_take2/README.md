...
`calloc`, `free`, `strlen`, `strcpy`, `memmove`, `memcpy`, and `memcmp`, including range,
return-value, zero-fill, overflow, allocation-reuse, string termination, and
...
service; at the current boundary that service is `getenv`.
