...
End:
Notes: Restored `mk_prepare_workspace_alt`, `mk_otccelf_alt`, and `mk_from_bootstrap_seed_alt` to the last known-good full-chain behavior: only `tcc_23/` is overlaid, `tcc_10` remains in the workspace, `mk_otccelf_alt` runs `x86/artifact/kaem-optional-seed ./alt_kaem.x86` plus `otccelf/mk_elf_loader`, and `mk_from_bootstrap_seed_alt` runs the copied workspace `x86/artifact/kaem-optional-seed`. Revalidated both entrypoints on the current branch tip: `mk_otccelf_alt` passes `sha256sum -c sum`, and `mk_from_bootstrap_seed_alt` passes both `sha256sum -c sum` and `mk_verify_tcc_27_boot_static`. Added host-build helpers `mk_gcc_tcc3_host_alt` and `mk_gcc_tcc10_host_alt` plus `tcc_10_host_strtod.c`; these build GCC-linked debug executables for `tcc_3` and `tcc_10` inside the ignored `_alt_work/host_debug/` area without changing the accepted bootstrap chain. The next task is to backport ELF object creation into `tcc_3`, then use both `tcc_3` and `tcc_10` to compile `tcc_23` as ELF objects and compare the generated machine code.
